...
# This script allows you to unpause non critical groups with dynamic membership, 100 at a time.
...
function UnpauseGroup {
    ...
    try {
        ...
        Invoke-MgGraphRequest -Uri $uri/$groupId -Method PATCH -Headers @{ConsistencyLevel = "eventual"} -Body $unpauseDGjson
...
# This function prompts you to confirm that you want to unpause non-critical groups with dynamic membership. 100 at a time.
# It reports the number of successfully unpaused and failed groups.
function UnpauseNonCriticalGroupsWithDynamicMembership {
    param (
        [string] $endpoint
    )
    #Prompt you to confirm if you want to proceed with unpausing 100 groups.
    ...
    Write-Host "YOU CAN ONLY UNPAUSE 100 GROUPS AT A TIME USING THIS SCRIPT." -ForegroundColor Yellow
    Write-Host "PLEASE MAKE SURE YOU HAVE ALREADY UNPAUSED THE CRITICAL GROUPS. THIS SCRIPT IS RECOMMENDED TO BE USED FOR NON-CRITICAL GROUPS ONLY." -ForegroundColor Yellow
    Write-Host "CONFIRM IF YOU WANT TO PROCEED WITH UNPAUSING 100 NON-CRITICAL GROUPS WITH DYNAMIC MEMBERSHIP." -ForegroundColor Yellow
    $input = Read-Host "Type 'yes' to confirm: "
    if ($input.Trim().ToLower() -eq "yes") {
        # Initialize URI for the first page
        $url = "$graphEndpoint/v1.0/groups"
        $filter = "?`$filter=groupTypes/any(c:c eq 'DynamicMembership') and membershipRuleProcessingState eq 'Paused'"
        $top = "&`$top=100"
        $uri = "$url$filter$top"
        ...
        # Unpause all groups page by page
        do {
            ...
            # Unpause each group in the current page
            ...
            $uri = $nextPage
            Write-Host "Checking if more groups are present on the next page." -ForegroundColor Yellow
        } while ($uri -ne $null)
        Write-Host "No more groups found." -ForegroundColor Yellow
        # Report the total count
        Write-Host "UnpauseNonCritical Operation Complete. Successfully Unpaused: $successCount, Failed: $failureCount" -ForegroundColor Yellow
    } else {
        Write-Host "UnpauseNonCritical script terminated." -ForegroundColor Red
        return
    }
}
...
Write-Host "DO YOU WANT TO UNPAUSE NON-CRTICIAL GROUPS WITH DYNAMIC MEMBERSHIP?" -ForegroundColor Yellow
$input = Read-Host "Type 'yes' to confirm: "
...
if (($input.Trim().ToLower() -eq "yes")) {
    ...
    try {
        ...
        UnpauseNonCriticalGroupsWithDynamicMembership -endpoint $graphEndpoint
    } catch {
        # Handle any errors during the process
        $errorMessage = $_.Exception.Message
        Write-Host "UnpauseNonCritical operation failed for group with dynamic membership. Error message: $($errorMessage)" -ForegroundColor Red
    }
