...
# This script allows you to pause all groups with dynamic membership.
...
function PauseAllGroupsWithDynamicMembership {
    param (
    ...
    Write-Host "PauseAll Operation Complete. Successfully Paused: $successCount, Failed: $failureCount" -ForegroundColor Yellow
}
...
Write-Host "DO YOU WANT TO PAUSE ALL GROUPS WITH DYNAMIC MEMBERSHIP?" -ForegroundColor Yellow
$input = Read-Host "Type 'yes' to confirm: "
...
if ($input.Trim().ToLower() -eq "yes") {
    ...
    try {
        ...
        # Pause all groups with dynamic membership
        PauseAllGroupsWithDynamicMembership -graphEndpoint $graphEndpoint
    } catch {
